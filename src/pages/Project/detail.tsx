  ...
  const CommentH5 = () => {
    return(
      <Container style={{minHeight: '80vh', padding:'0 15px'}} width={containerWidth}>
        <Row style={{height: '53px'}}>
          <LocalStyle.ProjectImgSearch src={require('../../assets/images/Icons/h5/left.png').default} onClick={() => {setComment(false)}}/>
          <Text color={theme.colors.text} fontWeight="bold" mr="20px" style={{flex: '1', textAlign: 'center'}}>Comment</Text>
        </Row>
        <LocalStyle.CommentLine />
        <Row>
        ...
        />
        <Button 
            style={{width: '100%', height: '48px', borderRadius:'24px !important', fontSize: '16px', fontWeight: 'bold'}} 
            type="primary"
            disabled={!title || !rate} 
            onClick={() => {confirmComment({ title, content, rate })}}>{t("Submit")}</Button>
      </Container>
    )
  }
